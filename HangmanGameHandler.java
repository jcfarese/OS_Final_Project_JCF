...
public class HangmanGameHandler implements Runnable{
	
	//Lists that are used to manage people playing a game and waiting in a lobby
	public static ArrayList<HangmanGameHandler> hangmanGameHandler = new ArrayList(); 
	public static ArrayList<HangmanGameHandler> waitingLobby = new ArrayList();
	...
	public HangmanGameHandler(Socket socket) {
	...
	public static void addToWaitingLobby(HangmanGameHandler handler) {
		lobbyLock.lock();
			try {
	...
	private static void movePlayersToGameHandler() {
		...
		try {
			for(int i=0; i < waitingLobbySize; i++) {
				HangmanGameHandler player = waitingLobby.remove(0);
	...
	public void broadcastMessage(String message) {
		for(HangmanGameHandler hangmanPlayer:hangmanGameHandler) {
			try {
